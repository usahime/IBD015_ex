...
select t.nome
from tbl_cliente as t
left join tbl_emprestimo l on t.codigo_cliente = l.numero_emprestimo
where l.numero_emprestimo is null

--4 Liste os títulos e suas categorias dos livros disponiveis (retorna 1 linha).

select t.titulo, t.categoria, l.status
from tbl_titulo as t
left join tbl_livros l on t.codigo_titulo = l.cod_livro
where l.status = 'DISPONIVEL'

--5 Liste os nomes dos clientes e os títulos dos livros que eles têm alugados (retorna 5 linhas).

select t.nome, l.titulo
from tbl_cliente as t
left join tbl_emprestimo z on t.codigo_cliente = z.codigo_cliente
left join tbl_titulo l on l.codigo_titulo = z.codigo_livro

--6 Retorne o nome, titulo do livro e o status do esmprestimo do livro alugado pela Ana Oliveira (retorna 1 linha)

select t.nome, l.titulo, z.status
from tbl_cliente as t
right join tbl_emprestimo w on t.codigo_cliente = w.codigo_cliente
right join tbl_titulo l on l.codigo_titulo = w.codigo_livro
right join tbl_livros z on z.cod_livro = w.codigo_livro
where t.nome = 'Ana Oliveira'